...
const listFoods = (recipe) => {
  let result = [];
    recipe.ingredients.forEach(function(element){
        ...
        let secondIndex = element.indexOf(' ',firstIndex + 1); 
        result.push(element.slice(secondIndex+ 1));
    })
  return result;
}
...
const splitFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(function(element){
      let splitIngredient = element.split(' ');
      let item = splitIngredient.slice(2);
      let thisItem = item.join(' ');
...
const stepActions = (recipe) => {
  let result = [];
    recipe.steps.forEach(function(element){
        let splitThis = element.indexOf(' ');
        result.push(splitThis[0]);
    })
