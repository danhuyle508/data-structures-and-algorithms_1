...
const count = (target, input) => {
  return input.reduce((acc,cur) =>{
    const inc = cur.reduce((accumulate, current)=>{
      if(current === target){
        return accumulate + 1;
      }
      return accumulate;
...
const totalSum = (input) => {
  return input.reduce((acc, cur)=>{
    let total = cur.reduce((accumulate, current)=>{
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arr =>{
    let newArr =arr.filter(filterOut=>{
      return filterOut%5 ===0 && typeof filterOut === "number";
    })
...
let findMaleAndFemale = (data) => {
  let filteredOut =data.filter( personObj =>{
   return personObj.gender === 'male' || personObj.gender === 'female'
  });
  return filteredOut.map(personObj=>{
...
let findShortest = (data) => {
  ...
  if(!accumulate.height || parseInt(accumulate.height) > parseInt(current.height)){
    return current;
  }
  return accumulate;
 }, {}).name;
